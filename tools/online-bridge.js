...
async function syncBusiness(){
  ...
  try{
    ...
    const conflicts=[];
    const sameFields=(a,b,fields)=>fields.every(field=>{const left=String(a?.[field]??''),right=String(b?.[field]??'');return field==='date'?left.slice(0,10)===right.slice(0,10):left===right;});
    ...
    const localTxById=new Map(localTx.map(x=>[String(x.id),x]));
    const localShById=new Map(localShifts.map(x=>[String(x.id),x]));
    for(const t of serverTx){
      const local=localTxById.get(String(t.id));
      if(local&&!sameFields(local,t,['date','customerId','serviceId','servicePrice','employeeId','total','payment','status','discount']))conflicts.push({type:'transaction',id:String(t.id),resolution:'server'});
    }
    for(const r of serverSh){
      const local=localShById.get(String(r.id));
      if(local&&!sameFields(local,r,['date','employeeId','shiftType','customers','openingCash','cash','qris','cashExpense','physicalCash','totalPayment','expectedCash','cashDifference','serviceTotal','productTotal','totalOmzet','note']))conflicts.push({type:'shiftReport',id:String(r.id),resolution:'server'});
    }
    ...
    save();
    window.WZOnlineBusiness.lastConflicts=conflicts;
